![JavaScript面试题.png](..\..\image\fc9240467b46494ca8fdc2d35d9f729e~tplv-k3u1fbpfcp-watermark.awebp)
...
console.log({} instanceof Object);                   // tru
```
...
下面看一下**双精度数是如何保存**的： ![img](..\..\image\0cb225cf71d748a8b2d6a5615e402711~tplv-k3u1fbpfcp-watermark.awebp)
...
其流程图如下： ![img](..\..\image\c451c19e23dd4726b3f36223b6c18a1e~tplv-k3u1fbpfcp-watermark.awebp)
...
最终，a、b、c分别被赋予了数组第0、1、2个索引位的值： ![img](..\..\image\e55fc36b191340e69698782fbd67ef4f~tplv-k3u1fbpfcp-watermark.awebp) 数组里的0、1、2索引位的元素值，精准地被映射到了左侧的第0、1、2个变量里去，这就是数组解构的工作模式。还可以通过给左侧变量数组设置空占位的方式，实现对数组中某几个元素的精准提取：
...
通过把中间位留空，可以顺利地把数组第一位和最后一位的值赋给 a、c 两个变量： ![img](..\..\image\a14ffbb3df2646a4a84f4a0c7d62d975~tplv-k3u1fbpfcp-watermark.awebp)
...
这样就得到了 name 和 age 两个和 stu 平级的变量： ![img](..\..\image\1ed2565845f2415b8243c8c355b2c6d6~tplv-k3u1fbpfcp-watermark.awebp)
...
一个对象系统的继承特性有三种实现方法：基于类、基于原型和基于元类。js是基于原型继承来实现对象系统的。

首先原型的含义是指：如果构造器（Object()）有一个原型对象，因为原型对象有的属性方法在实例对象上都可以找到，我们可以判断出该构造器创建的实例都必然复制自该原型对象。换言之，所谓的原型就是构造器用于生成实例的模板。而这样的“复制”就存在多种可能性，由此引申出动态绑定和静态绑定等问题。



原型链：就是对象所有的父类和祖先类的原型所形成的、可上溯访问的链表。
...
**特点：** JavaScript 对象是通过引用来传递的，创建的每个新对象实体中并没有一份属于自己的原型副本。当修改原型时，与之相关的对象也会继承这一改变。 ![img](..\..\image\c68fcad75ea54d62a9404aa02cafc65c~tplv-k3u1fbpfcp-watermark.awebp)
...
由于`Object`是构造函数，原型链终点是`Object.prototype.__proto__`，而`Object.prototype.__proto__=== null // true`，所以，原型链的终点是`null`。原型链上的所有原型都是对象，所有的对象最终都是由`Object`构造的，而`Object.prototype`的下一级是`Object.prototype.__proto__`。 ![img](..\..\image\30821909377545248d244b0a4e965bb1~tplv-k3u1fbpfcp-watermark.awebp)
...
+ promise是什么
  + promise是一种异步编程的解决方案。ES6将其写进了语言标准，并提供了一个Promise对象
+ promise是为了解决什么问题而产生的
  + ES6之前处理异步函数是通过回调函数来处理的，如果多个异步任务相互依赖，这样就会导致一个回调函数里面嵌另一个回调函数，一直嵌套下去直到没有依赖上一个回调函数的异步任务。这样就导致了回调地域。为了解决这个问题，promise产生了。
+ promise状态的两个特点
  + promise对象的状态不受外界影响，并且一旦改变就不能再变，且只能由pending变为fulfilled或者由pending变为rejected
    + pending 初始状态
    + fulfilled 成功状态
    + rejected 失败状态
+ promise的缺点
  + 无法取消promise，一旦新建就会立即执行，无法中途取消。
  + 如果不设置回调函数，promise内部返回的错误不会反映到外部。
...
![img](..\..\image\40194bdc07024b78b6da8bb43d84a7d8~tplv-k3u1fbpfcp-watermark.awebp) func1的运行结果其实就是一个Promise对象。因此也可以使用then来处理后续逻辑。
...
![img](..\..\image\507e439801c043739c4667567e5f8562~tplv-k3u1fbpfcp-watermark.awebp)
...
+ 当一个常量，变量或者对象不需要使用本应该被垃圾回收机制回收是，另一个正在使用的对象持有它的引用导致它没有被回收，这导致本该被回收的对象不能被回收而停留在堆内存中，这就产生了内存泄漏。
